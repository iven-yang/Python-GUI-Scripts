...
def check_post_game() -> bool:
    ...
    play_again_button = pyautogui.locateCenterOnScreen(os.path.join(images_folder, 'play_again.png'))
...
def play_again():
    ...
    while timeout > datetime.now():
        ...
        if play_again_button is not None:
            time.sleep(1)
            MouseClick.left_click(play_again_button[0], play_again_button[0], play_again_button[1], play_again_button[1])
            time.sleep(5)
            break
    else:
...
def find_match():
    ...
    while timeout > datetime.now():
        find_match_button = pyautogui.locateCenterOnScreen(os.path.join(images_folder, 'find_match.png'))
        if find_match_button is not None:
            MouseClick.left_click(find_match_button[0], find_match_button[0], find_match_button[1], find_match_button[1])
            break
    else:
